...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/submit_experimental_quest.sh [options]
...
RUN_SCRIPT="${REPO_ROOT}/scripts/run_experimental_quest.sh"
SBATCH_BIN="${SBATCH_BIN:-sbatch}"

RUN_ID="${run_id:-RUN-EXP-0002}"
CONFIG_PATH="${config_path:-${REPO_ROOT}/configs/experimental_small_qwen.yaml}"
...
OUTPUT_DIR="${output_dir:-${REPO_ROOT}/outputs/experimental/RUN-EXP-0002_small_qwen}"

mkdir -p "${OUTPUT_DIR}"

printf -v wrapped_cmd '%q ' \
  bash "${RUN_SCRIPT}" "${OUTPUT_DIR}" "${CONFIG_PATH}" "${INPUT_JSONL}" "${MANIFEST_PATH}"
